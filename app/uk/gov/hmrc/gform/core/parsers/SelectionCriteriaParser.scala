...
import uk.gov.hmrc.gform.sharedmodel.formtemplate.{ CsvColumnName, FormCtx, SelectionCriteriaValue }
...
object SelectionCriteriaParser {
  ...
  lazy val selectionCriteria = (
    ExprParsers.expr ^^ { (_, expr) =>
      ...
      |
        FormatParser.fieldId ~ "." ~ FormatParser.contextField ^^ { (_, id, _, column) =>
          SelectionCriteriaReference(FormCtx(id), CsvColumnName(column))
        } |
